...
delay(8);
  ...
  // setup motor to a know position of off and down
...
// resistacce is futile!! 
// if ((currentMotionState == M_DOWN) || (currentMotionState == M_UP)) 
//     {
...
delay(8);
  autohome.loop();
delay(8);
// 1 ms deylay to handle wifif ?

 // delay(20); // im a bad person.
 ...
 if ((millis() - lastMills) > 10)     // sneeky way to do timed events in seconds without hanging the CPU. 
    {
 ...
 //   if ( (currentValue != lastValue) || (currentValue != (lastValue -1)) || (currentValue != (lastValue +1)) && currentMotionState == M_STOP)  // detect change in position
          ...
          // added some tolerence due to poor DAC quality.
              ...
              valOut = HIGH_END;
              ...
              else if (valOut < LOW_END) 
                {
                  ...
                  autohome.sendPacket(out.c_str());
                       ...
                       Serial.println("switch is not flicked at pos ");         // debug things
                       Serial.print(out);
 /*               
